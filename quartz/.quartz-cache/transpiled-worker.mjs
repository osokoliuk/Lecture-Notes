...
var Citations = /* @__PURE__ */ __name((userOpts) => {
...
var countedCallouts = ["definition", "lemma", "theorem", "corollary", "example", "claim", "proposition", "exercise", "remark", "fact", "conjecture"];
var calloutContractions = { "definition": "Def", "lemma": "Lem", "theorem": "Thm", "corollary": "Cor", "example": "Exm", "claim": "Clm", "proposition": "Prp", "exercise": "Exr", "remark": "Rmk", "fact": "Fact", "conjecture": "Cnj" };
...
var ObsidianFlavoredMarkdown = /* @__PURE__ */ __name((userOpts) => {
  ...
  return {
    ...
    markdownPlugins(_ctx) {
      ...
      plugins.push(() => {
        return async (tree, _file) => {
          const cacheBaseUrl = "/latex-cache";
          const targets = [];
          ...
          for (let i = targets.length - 1; i >= 0; i--) {
            ...
            try {
              const m = node.value.match(/^\s*%+\s*latex-id\s*:\s*([0-9a-fA-F-]+)\s*$/m) || node.value.match(/latex-id\s*:\s*([0-9a-fA-F-]+)/i);
              if (!m) {
    ...
    htmlPlugins(_ctx) {
      ...
      plugins.push(() => {
        return (tree, _file) => {
          let note_number = "";
          if (_file.data.frontmatter && _file.data.frontmatter.id)
            note_number = _file.data.frontmatter.id + ".";
          let counter = 0;
          visit3(tree, "element", (node) => {
            if (node.tagName === "blockquote" && countedCallouts.includes(node.properties.dataCallout)) {
              counter++;
              node.properties.calloutNumber = counter;
              visit3(node, "element", (childNode) => {
                if (childNode.tagName === "div" && childNode.properties.className && childNode.properties.className.includes("callout-title-inner")) {
                  ...
                  if (pTag && pTag.type === "element" && pTag.tagName === "p") {
                    ...
                    if (childNode.children[0].children[0].type === "text") {
                      ...
                      if (prevVal.toLowerCase() == node.properties.dataCallout.toLowerCase())
                        displayDesc = false;
                      else
          ...
          visit3(tree, "element", (node) => {
            if (node.tagName === "svg" && node.properties.dataTikzSvg === "true") {
              if (node.properties.className)
                node.properties.className.append("TikzSvg");
              else
                node.properties.className = ["TikzSvg"];
      ...
      plugins.push(() => {
        return (tree, _file) => {
          const NumberNodeDict = /* @__PURE__ */ new Map();
          visit3(tree, "element", (node) => {
            if (node.tagName === "blockquote" && countedCallouts.includes(node.properties.dataCallout) && node.properties.id) {
          ...
          __name(afterPercent5E, "afterPercent5E");
          visit3(tree, "element", (node) => {
      ...
      plugins.push(() => {
        return (tree, _file) => {
          visit3(tree, "element", (node, index, parent) => {
            if (node.tagName === "pre" && node.properties.id && node.children && node.children[0] && node.children[0].tagName === "code" && node.children[0].properties.className.includes("math-display")) {
              let desc = {
                type: "element",
                tagName: "span",
...
// quartz/plugins/transformers/textTransforms.ts
var TextTransforms = /* @__PURE__ */ __name(() => {
  return {
    name: "TextTransforms",
    textTransform(_ctx, src) {
      if (src instanceof Buffer) {
      ...
      src = src.replace(/\\begin{align}/g, "\\begin{align*}").replace(/\\end{align}/g, "\\end{align*}").replace(/!!@!@!@!!!@!@!@@!@!@!@@/, "Last updated on: " + Date());
      return src;
    }
  };
}, "TextTransforms");

// quartz/plugins/transformers/numberEquations.ts
import rehypeRaw2 from "rehype-raw";
import { visit as visit6 } from "unist-util-visit";
var NumberEquaitons = /* @__PURE__ */ __name(() => {
  return {
    name: "NumberEquations",
    htmlPlugins() {
      const plugins = [rehypeRaw2];
      plugins.push(() => {
        return (tree, _file) => {
          const EquationDict = /* @__PURE__ */ new Map();
          let counter = 0;
          visit6(tree, "element", (node, index, parent) => {
            if (node.tagName === "span" && node.properties.id && node.properties.className.includes("numbered-equation-locator")) {
              counter++;
          ...
          visit6(tree, "element", (link) => {
            if (link.tagName === "a" && link.properties.href.slice(0, 4) === "#%5E") {
...
var ContentMeta_default = /* @__PURE__ */ __name((opts) => {
  ...
  function ContentMetadata({ cfg, fileData, displayClass }) {
    ...
    if (text) {
      ...
      if (fileData.frontmatter) {
        if (fileData.frontmatter.time)
          segments.push(fileData.frontmatter.time);
      }
      if (fileData.frontmatter) {
        if (fileData.frontmatter.id)
          segments.push("#" + fileData.frontmatter.id);
      }
...
var Footer_default = /* @__PURE__ */ __name((opts) => {
  const Footer = /* @__PURE__ */ __name(({ displayClass, cfg }) => {
    ...
    return /* @__PURE__ */ jsxs15("footer", { class: `${displayClass ?? ""}`, children: [
      /* @__PURE__ */ jsxs15("svg", { xmlns: "http://www.w3.org/2000/svg", width: "100%", height: "13", viewBox: "0 0 1300 13", preserveAspectRatio: "none", stroke: "var(--lightgray)", "stroke-width": "1", "aria-hidden": "true", focusable: "false", children: [
        /* @__PURE__ */ jsx26("line", { x1: "0", x2: "1300", y1: "0.5", y2: "0.5" }),
        /* @__PURE__ */ jsx26("line", { x1: "0", x2: "1300", y1: "4.5", y2: "4.5" }),
        /* @__PURE__ */ jsx26("line", { x1: "0", x2: "1300", y1: "8.5", y2: "8.5" }),
        /* @__PURE__ */ jsx26("line", { x1: "0", x2: "1300", y1: "12.5", y2: "12.5" })
      ] }),
      /* @__PURE__ */ jsxs15("div", { class: "wrapper", children: [
        /* @__PURE__ */ jsxs15("div", { class: "footer-text", children: [
        ...
        /* @__PURE__ */ jsxs15("div", { class: "license-notice", children: [
          /* @__PURE__ */ jsx26("div", { children: /* @__PURE__ */ jsx26("a", { href: "https://creativecommons.org/licenses/by-nc-sa/4.0/", children: "CC BY-NC-SA 4.0" }) }),
          /* @__PURE__ */ jsx26("img", { class: "noAI", src: "https://100r.co/media/interface/noai.png", alt: "noAI" })
        ] })
      ] })
    ] });
  }, "Footer");
...
var defaultContentPageLayout = {
  ...
  left: [
    ...
    Search_default(),
    Darkmode_default(),
    DesktopOnly_default(Graph_default({
      localGraph: {
        showTags: false,
        enableRadial: true
      },
...
async function parseFiles(buildId, argv, fps, allSlugs, mapping) {
  const ctx = {
    buildId,
